...
from schema1 import ExtractionResult, FieldEvidence, FormFieldUpdate, PromptContext
import json
import os
import requests
...
# ---------------------------------------------------------------------------
# LLM client wrapper for Gemma‑4 model via HTTP endpoint
def _llm_chat(messages: list[dict], model: str | None = None) -> str:
    """Send a chat request to a qwen2.5:7b  model.
    The endpoint URL and optional API key are read from environment variables:
        LLM_ENDPOINT – full URL of the chat endpoint (e.g. http://localhost:8000/v1/chat)
        LLM_API_KEY  – optional bearer token.
    The payload follows the OpenAI‑compatible schema.
    """
    endpoint = os.getenv("GEMMA_ENDPOINT", "http://localhost:11434/api/chat")
    api_key = os.getenv("GEMMA_API_KEY")
    parsed = urlparse(endpoint)
    host = (parsed.hostname or "").lower()
    if host not in {"localhost", "127.0.0.1", "::1"}:
        raise RuntimeError(
            f"Only local Ollama endpoint is allowed. Invalid GEMMA_ENDPOINT host: '{host}'"
        )
    resolved_model = model or os.getenv("LLM_MODEL", "qwen2.5:7b")
    payload = {
    ...
    raise RuntimeError("Unexpected LLM response format from endpoint")
